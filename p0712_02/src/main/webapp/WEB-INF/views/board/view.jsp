    ...
    <table>
      ...
      <tr>
        ...
        <c:if test="${map.nextDto.bno != null}"> 
        	<a href="/board/view?bno=${map.nextDto.bno }&page=${page}">[게시글] ${map.nextDto.btitle }</a></td>
       	</c:if>
        <c:if test="${map.nextDto.bno == null}">
        	다음글이 없습니다.
        </c:if> 
      ...
      <tr>
        ...
        <c:if test="${map.prevDto.bno != null}">         	
        	<a href="/board/view?bno=${map.prevDto.bno }&page=${page}">[게시글] ${map.prevDto.btitle }</a></td>
       	</c:if>
        <c:if test="${map.prevDto.bno == null}">
        	이전글이 없습니다.
        </c:if>         	
