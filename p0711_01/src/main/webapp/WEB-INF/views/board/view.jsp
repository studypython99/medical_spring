...
<meta charset="UTF-8">
  ...
  <script>
  	function deleteBtn(bno){
  		//alert(bno); //게시글 번호가 뜬다, 확인하면서 작성하자
  		if(confirm("게시글을 삭제하시겠습니까?")){
  			location.href="/board/delete?bno="+bno; //파라미터값으로 데이터를 넘긴다
  		}//if
  	}
  </script>
</head>
    ...
    <table>
      ...
      <c:if test="${boardDto.bfile != null }">
        <td colspan="3" class="article">
        	<img style="whdth:50%;" src="/upload/${boardDto.bfile }">
       	</td>
      </c:if>
      <c:if test="${boardDto.bfile == null }">
        <td colspan="3" class="article">파일첨부 없음</td>
        </c:if>
      </tr>
      <tr>
